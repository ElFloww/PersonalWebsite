                ...
                <v-col v-for="(stat, index) in stats" :key="index" cols="12" sm="6" md="4">
                    <div class="stat-card" :style="{ animationDelay: `${index * 0.1}s` }">
                        <div class="stat-icon-wrapper" :style="{ background: stat.gradient }">
                            <v-icon :icon="stat.icon" size="48" color="white" class="floating"></v-icon>
                        </div>
                        <div class="stat-content">
                            <div class="stat-number gradient-text">
                                {{ stat.value }}{{ stat.suffix }}
                            </div>
                            <div class="stat-label">{{ t(`views.main.stats.${stat.label}`) }}</div>
                            <div class="stat-sublabel">{{ t(`views.main.stats.${stat.sublabel}`) }}</div>
                        </div>
